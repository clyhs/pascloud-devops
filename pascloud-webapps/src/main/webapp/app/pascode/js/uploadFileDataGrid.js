...
function createUploadDataGridDialog(){
    var div = '';
    
    div +='<div class="easyui-layout" data-options="fit:true">';  
    div +='<div data-options="region:\'center\'">';
    div +='    <div style="height: 27px;" data-options="border:false">';
    div +='        <div  style="float: left;height: 25px;margin-right: 3px;">';
    div +='            <div  id="chooseFile">选择文件</div>';
    div +='        </div>';
    div +='        <a class="easyui-linkbutton" id="removeUpFile" data-options="iconCls:\'icon-remove\'" style="height: 27px;width: 100px;" onclick="removeFile();">移除文件</a>';
    div +='        <a class="easyui-linkbutton" id="addUpFile" data-options="iconCls:\'icon-add\'" style="height: 27px;width: 100px;" onclick="uploadToServer();">开始上传</a>';
    div +='     </div>';
    div +='    <table id="upfileGrid"></table>';
    div +='</div>';
    div +='</div>';
...
function uploadRegister(){
	...
	uploader = WebUploader.create({
        ...
        // 默认文件接收服务端。
        server: '/module/pascode/uploadPascode.json',
        // 选择文件的按钮。可选。
        // 内部根据当前运行是创建，可能是input元素，也可能是flash.
        pick: '#chooseFile',
        //formData: { "dirId": dirId},
        //formData: { "dirId": 'dn0'},
        fileSingleSizeLimit:300*1024*1024,//单个文件大小
        accept:[{
            title: 'file',
            extensions: 'gz,war',
            mimeTypes: '.gz,.war'
