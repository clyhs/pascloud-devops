...
import com.pascloud.utils.JavaBeanToMapUtil;
import com.pascloud.utils.JsonUtils;
...
//	private  String zookeeper;
	...
	public void createEphemeralNode(String path, String data) throws Exception {
		...
		client.create().creatingParentsIfNeeded()
				...
				.forPath(path, data.getBytes(Constants.CHARSET));
	}
	...
	public String getNodeData (String path) throws Exception {
		...
		String rep = new String(byteData, Constants.CHARSET);
		return rep;
	}
	...
	public String getNodeData(String path, Stat stat) throws Exception {
		...
		return new String(byteData, Constants.CHARSET);
	}

	public void setNodeData(String path, String data) throws Exception {
		Preconditions.checkNotNull(client, errorWithNullClient);
		client.setData().forPath(path, data.getBytes(Constants.CHARSET));
	...
	public <T> T getBeanData(String path,Class<T> claz) throws Exception {
		...
		String rep = null;
		byte[] byteData = client.getData().forPath(path);
		rep = new String(byteData, Constants.CHARSET);
	...
	public boolean createMainPath() throws Exception {
		if (!existsNode(Constants.MYCAT_EYE)) {
			createNode(Constants.MYCAT_EYE,"mycat eye");
			createNode(Constants.MYCATS,"mycat node");
			createNode(Constants.MYCAT_JMX,"jmx");
			createNode(Constants.MYCAT_MYSQL,"mysql");
			createNode(Constants.MYCAT_SNMP,"snmp");
			createNode(Constants.MYCAT_PROCESSOR,"processor");
		}
		return true;
	}
