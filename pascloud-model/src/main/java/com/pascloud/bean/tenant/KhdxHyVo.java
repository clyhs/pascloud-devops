package com.pascloud.bean.tenant;

import java.io.Serializable;

public class KhdxHyVo implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	
	private Integer khdxdh;
	
	private String  hydh;
	
	private String  hymc;
	
	private String  xl;
	
	private String  lxdh;
	
	private String  sfz;
	
	private String  yxrybz;
	
	private String  xnhybz;
	
	private String  dlmc;
	
	private String  dlmm;
	
	private String  aqjb;
	
	private String  zxzt;
	
	private String  scdl;
	
	private String  zpxx;
	
	private String  czybh;
	
	private Integer zxrq;
	
	private Integer csrq;
	
	private Integer gzrq;
	
	private Integer rhrq;
	
	private String  fgbz;
	
	private Integer pxbz;
	
	private String  fhdh;

	public Integer getKhdxdh() {
		return khdxdh;
	}

	public void setKhdxdh(Integer khdxdh) {
		this.khdxdh = khdxdh;
	}

	public String getHydh() {
		return hydh;
	}

	public void setHydh(String hydh) {
		this.hydh = hydh;
	}

	public String getHymc() {
		return hymc;
	}

	public void setHymc(String hymc) {
		this.hymc = hymc;
	}

	public String getXl() {
		return xl;
	}

	public void setXl(String xl) {
		this.xl = xl;
	}

	public String getLxdh() {
		return lxdh;
	}

	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}

	public String getSfz() {
		return sfz;
	}

	public void setSfz(String sfz) {
		this.sfz = sfz;
	}

	public String getYxrybz() {
		return yxrybz;
	}

	public void setYxrybz(String yxrybz) {
		this.yxrybz = yxrybz;
	}

	public String getXnhybz() {
		return xnhybz;
	}

	public void setXnhybz(String xnhybz) {
		this.xnhybz = xnhybz;
	}

	public String getDlmc() {
		return dlmc;
	}

	public void setDlmc(String dlmc) {
		this.dlmc = dlmc;
	}

	public String getDlmm() {
		return dlmm;
	}

	public void setDlmm(String dlmm) {
		this.dlmm = dlmm;
	}

	public String getAqjb() {
		return aqjb;
	}

	public void setAqjb(String aqjb) {
		this.aqjb = aqjb;
	}

	public String getZxzt() {
		return zxzt;
	}

	public void setZxzt(String zxzt) {
		this.zxzt = zxzt;
	}

	public String getScdl() {
		return scdl;
	}

	public void setScdl(String scdl) {
		this.scdl = scdl;
	}

	public String getZpxx() {
		return zpxx;
	}

	public void setZpxx(String zpxx) {
		this.zpxx = zpxx;
	}

	public String getCzybh() {
		return czybh;
	}

	public void setCzybh(String czybh) {
		this.czybh = czybh;
	}

	public Integer getZxrq() {
		return zxrq;
	}

	public void setZxrq(Integer zxrq) {
		this.zxrq = zxrq;
	}

	public Integer getCsrq() {
		return csrq;
	}

	public void setCsrq(Integer csrq) {
		this.csrq = csrq;
	}

	public Integer getGzrq() {
		return gzrq;
	}

	public void setGzrq(Integer gzrq) {
		this.gzrq = gzrq;
	}

	public Integer getRhrq() {
		return rhrq;
	}

	public void setRhrq(Integer rhrq) {
		this.rhrq = rhrq;
	}

	public String getFgbz() {
		return fgbz;
	}

	public void setFgbz(String fgbz) {
		this.fgbz = fgbz;
	}

	public Integer getPxbz() {
		return pxbz;
	}

	public void setPxbz(Integer pxbz) {
		this.pxbz = pxbz;
	}

	public String getFhdh() {
		return fhdh;
	}

	public void setFhdh(String fhdh) {
		this.fhdh = fhdh;
	}
	
	

}
